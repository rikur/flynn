...
func (s *DomainMigrationSuite) migrateDomain(t *c.C, dm *ct.DomainMigration) {
	...
	doPing = func(url string, retriesRemaining int) {
		res, err := (&http.Client{}).Get(url)
		if (err != nil || res.StatusCode != 200) && retriesRemaining > 0 {
			doPing(url, retriesRemaining-1)
			return
		}
		t.Assert(err, c.IsNil)
		t.Assert(res.StatusCode, c.Equals, 200, c.Commentf("failed to ping controller"))
	}
	doPing(fmt.Sprintf("http://controller.%s/ping", dm.Domain), 3)
	doPing(fmt.Sprintf("http://dashboard.%s/ping", dm.Domain), 3)
}
